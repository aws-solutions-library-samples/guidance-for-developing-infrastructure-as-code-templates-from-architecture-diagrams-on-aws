...
- [Preparing Generated Code for Deployment with Kiro](#preparing-generated-code-for-deployment-with-kiro)
...
## Preparing Generated Code for Deployment with Kiro

The CDK stacks generated by A2A-AI include infrastructure definitions and a `resource_spec.json` file that describes every resource requiring application code (Lambda handlers, ECS containers, Step Function definitions, etc.). Kiro can use this spec to automatically scaffold a complete, deployable CDK project with working application code — not just stubs.

### Prerequisites
- [Kiro CLI](https://kiro.dev) installed
- The downloaded code package from A2A-AI extracted locally

### Steps

1. Extract the generated code package and navigate to the directory:
   ```bash
   unzip a2a-output.zip -d my-project
   cd my-project
   ```
...
3. Run the prepare-cdk workflow:
   ```
   /chat load docs/prepare-cdk.md
   ```

4. Kiro will automatically:
   - Analyze the generated CDK stack files and identify the code language
   - Initialize a new CDK project with proper structure
   - Migrate and adapt all stack files with corrected imports
   - Read `resource_spec.json` and generate complete application code for each resource:
     - Lambda handlers with full business logic based on the `operations` and `logic` fields
     - ECS container applications with Dockerfiles and route implementations
     - Step Function ASL definitions
     - Custom resource handlers
   - Install dependencies and fix imports
   - Run `cdk synth` to verify everything compiles

### What resource_spec.json Provides

The spec includes detailed metadata for each resource that Kiro uses to write real implementations:

- **operations** — Each discrete action with input/output schemas and step-by-step `logic` descriptions
- **data_models** — Table schemas (DynamoDB keys/attributes), graph models (Neptune vertices/edges), search indexes (OpenSearch field mappings)
- **dependencies** — Required packages beyond the standard AWS SDK
- **connection_config** — Connection management guidance for services like Neptune and OpenSearch
- **error_handling** — Expected error response format per function
- **vpc** — Whether the function runs in a VPC (informs connection initialization patterns)

### Post-Workflow Review

After Kiro completes the workflow:
1. Review the generated application code in `src/lambda-functions/`, `src/containers/`, and `src/step-functions/`
2. Verify environment variable names match between CDK stacks and handler code
3. Run `cdk synth` to confirm successful synthesis
